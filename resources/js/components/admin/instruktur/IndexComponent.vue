      ...
      <div class="container-xl">
        <div class="row g-2 align-items-center">
          ...
          <div class="col-auto ms-auto d-print-none">
            <div class="btn-list">
              <router-link
                :to="{ name: 'admin.instruktur.create' }"
            ...
            <div class="card-body">
              ...
              <div class="table-responsive">
                <table class="table table-vcenter table-hover">
                  ...
                  <tbody>
                    <tr v-for="(instruktur, index) in instrukturs" :key="instruktur.id">
                      <td>
                        {{
                          meta.current_page * meta.per_page - meta.per_page + index + 1
                        }}
                      </td>
                      <td style="text-wrap: nowrap">{{ instruktur.user.name }}</td>
                      <td style="text-wrap: nowrap">{{ instruktur.user.email }}</td>
                      <td style="text-wrap: nowrap">{{ instruktur.user.phone }}</td>
                      <td style="text-wrap: nowrap">{{ instruktur.user.alamat }}</td>
                      <td style="text-wrap: nowrap">{{ instruktur.pendidikan_strtoupper }}</td>
                      <td style="text-wrap: nowrap">{{ instruktur.honor_perjam_rp }}</td>
    ...
    deleteInstruktur(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          Loading();
          axios
            .post("/v1/instruktur/delete/" + id)
            .then((response) => {
              const data = response.data;
              AlertMsg(data.message, data.error);
              if (!data.error) {
                this.fetchInstrukturData(this.meta.current_page);
              }
            })
            .catch((error) => {
              AlertMsg(error.response.data.message, true);
