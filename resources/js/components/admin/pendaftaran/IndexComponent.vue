                ...
                <table class="table table-vcenter table-hover">
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td style="text-wrap: nowrap">{{ pendaftaran.kode_pendaftaran }}</td>
                      <td style="text-wrap: nowrap">{{ pendaftaran.peserta.user.name }}</td>
                      <td style="text-wrap: nowrap">{{ pendaftaran.program.nama_program }}</td>
                      <td style="text-wrap: nowrap">{{ pendaftaran.kelas.nama_kelas }}</td>
                      <td style="text-wrap: nowrap">
                        <router-link
                          :to="{
                            name: 'admin.pendaftaran.edit',
                            params: { id: pendaftaran.id },
                          }"
                        ...
                        >
                        <!-- <button
                          @click="deletePendaftaran(pendaftaran.id)"
    ...
    deletePendaftaran(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          Loading();
          axios
            .post("/v1/pendaftaran/delete/" + id)
            .then((response) => {
              const data = response.data;
              AlertMsg(data.message, data.error);
              if (!data.error) {
                this.fetchPendaftaranData(this.meta.current_page);
              }
            })
            .catch((error) => {
              AlertMsg(error.response.data.message, true);
